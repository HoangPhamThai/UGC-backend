# app/modules/workspaces/domain/usecases/update_feedback.py
from dataclasses import dataclass

from app.core.logging_mixin import LoggerMixin
from app.modules.users.data.model import User
from app.modules.workspaces.data.model import AWAITING_QC_STATUSES, Feedback, FeedbackStatus
...
class UpdateFeedbackUseCase(LoggerMixin):
    ...
    async def execute(
        ...
        *,
        ...
        cleaned = body.strip()
        if not cleaned:
            raise ArticleStateConflictError("Feedback body cannot be empty")
        ...
        if feedback.status != FeedbackStatus.DRAFT:
            raise FeedbackStateConflictError("Only draft feedback can be edited")
        if feedback.author_id != caller.id:
            raise FeedbackStateConflictError("Only the author can edit this feedback")

        updated = await self.feedback_repo.update_body(feedback_id, cleaned)
        if updated is None:
            raise FeedbackNotFoundError()
        return updated
